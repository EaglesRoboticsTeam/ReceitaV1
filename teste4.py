...
BP.set_sensor_type(BP.PORT_1, BP.SENSOR_TYPE.EV3_REFLECTED) # Configure for a touch sensor. If an EV3 touch sensor is connected, it will be configured for EV3 touch, otherwise it'll configured for NXT touch.
...
try:
    ...
    while True:
        ...
        if value:                             # if the touch sensor is pressed
            if speed <= -0 or speed >= 0: # if speed reached 100, start ramping down. If speed reached -100, start ramping up.
                adder = -adder
            speed += adder
